...
st.write(
    "Upload reference images (ZIP) and a query image. This tool compares them using "
    "structure, color, texture, edge, entropy, and hue metrics, and generates three "
    "different intersection palettes to visualize shared chromatic structure."
)
...
def kmeans_palette(img, k):
    ...
    return np.clip(centers[:k], 0, 255).astype(np.uint8)
...
def entropy_similarity(a,b):
    a = cv2.calcHist([np.array(a.convert("L"))],[0],None,[256],[0,256])
    b = cv2.calcHist([np.array(b.convert("L"))],[0],None,[256],[0,256])
    a=a/(a.sum()+1e-8); b=b/(b.sum()+1e-8)
    ea = -np.sum(a*np.log2(a+1e-12))
    eb = -np.sum(b*np.log2(b+1e-12))
    return float(np.clip(1 - abs(ea-eb)/max(ea,eb,1e-6),0,1))
...
def texture_glcm_similarity(a,b):
    a = np.array(a.convert("L"))
    b = np.array(b.convert("L"))
    ga = graycomatrix(a,[5],[0],symmetric=True,normed=True)
    gb = graycomatrix(b,[5],[0],symmetric=True,normed=True)
    ca = graycoprops(ga,'contrast')[0,0]
    cb = graycoprops(gb,'contrast')[0,0]
    return float(np.clip(1 - abs(ca-cb)/max(ca,cb,1e-6),0,1))

def hue_distribution_similarity(a,b,bins,s_thr,v_thr):
    a = cv2.cvtColor(np.array(a),cv2.COLOR_RGB2HSV)
    b = cv2.cvtColor(np.array(b),cv2.COLOR_RGB2HSV)
    ma = (a[...,1]>=s_thr)&(a[...,2]>=v_thr)
    mb = (b[...,1]>=s_thr)&(b[...,2]>=v_thr)
    ha = a[...,0][ma]; hb = b[...,0][mb]
    if ha.size==0 or hb.size==0: return 0.5
...
# ---------------- Distinct Palette Algorithms ---------------- #
def blended_midpoint_palette(q_cols, r_cols, k):
    q = rgb2lab(q_cols.reshape(-1,1,3)/255).reshape(-1,3)
    r = rgb2lab(r_cols.reshape(-1,1,3)/255).reshape(-1,3)
    out=[]
    for i in range(min(k,len(q))):
        j=np.argmin(np.linalg.norm(r-q[i],axis=1))
        mid = (q[i]+r[j])/2
...
def shared_hue_intersection_palette(q_cols, r_cols, k, bins, s_thr, v_thr):
    # convert to HSV
    hq = cv2.cvtColor(q_cols.reshape(1,-1,3),cv2.COLOR_RGB2HSV).reshape(-1,3)
    hr = cv2.cvtColor(r_cols.reshape(1,-1,3),cv2.COLOR_RGB2HSV).reshape(-1,3)

    mask_q = (hq[:,1]>=s_thr)&(hq[:,2]>=v_thr)
    mask_r = (hr[:,1]>=s_thr)&(hr[:,2]>=v_thr)

    hq_bins = (hq[:,0]/(180/bins)).astype(int)[mask_q]
    hr_bins = (hr[:,0]/(180/bins)).astype(int)[mask_r]

    shared = set(hq_bins).intersection(set(hr_bins))
    if not shared:
        return blended_midpoint_palette(q_cols,r_cols,k)

    out=[]
    for b in sorted(shared):
        q_match = q_cols[np.where(hq_bins==b)[0]]
        r_match = r_cols[np.where(hr_bins==b)[0]]
        qc=np.mean(q_match,axis=0); rc=np.mean(r_match,axis=0)
        out.append((qc+rc)/2)

    out = np.array(out)
    if out.shape[0]>k: out=out[:k]
    while out.shape[0]<k:
        out=np.vstack([out,q_cols[0]])
...
def plotly_palette(colors):
    hex_cols=[f"#{c[0]:02x}{c[1]:02x}{c[2]:02x}" for c in colors]
    fig=go.Figure(go.Bar(
        x=[1]*len(colors),
        y=[1]*len(colors),
        marker=dict(color=hex_cols),
        hovertext=hex_cols,
        hoverinfo="text",
        orientation="h"))
    fig.update_layout(
...
if uploaded_zip and query_file:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for idx,(path,img,metrics,score) in enumerate(top,1):
            st.markdown(f"### Match {idx}: {os.path.basename(path)} — {score:.2f}")
            c1,c2=st.columns([2.5,1.3],gap="large")
            ...
            with c1:
                ...
                fig.update_layout(
                    ...
                    showlegend=False)
                st.plotly_chart(fig,use_container_width=True,key=f"metrics_{idx}")
            ...
            with c2:
                ...
                st.plotly_chart(plotly_palette(mid), use_container_width=True, key=f"mid_{idx}")

                st.markdown("Shared Hue Intersection")
                st.plotly_chart(plotly_palette(inter), use_container_width=True, key=f"inter_{idx}")

                st.markdown("Weighted Hybrid")
                st.plotly_chart(plotly_palette(hybrid), use_container_width=True, key=f"hybrid_{idx}")
